...
def _get_train_target_ats(model, x_train, x_target, target_name, layer_names, args):
    ...
    output = layer.output
    fun = K.function([model.input], [output])
    train_ats = fun([x_train])[0]
    x_target = x_target.reshape(900, 16, 16, 1)
    target_ats = fun([x_target])[0]
    train_pred = np.argmax(model.predict(x_train), axis=1)
    target_pred = np.argmax(model.predict(x_target), axis=1)
